...
def _find_targets(
    ...
    match node:
        ...
        case _:
            assert (
                False
            ), f"Unsupported node for target searching - {node.__class__.__name__}"
...
class TypeHintTransformer(codemod.ContextAwareTransformer):
    ...
    def _get_trace_for_targets(
        ...
        if not len(containing_classes):
            ...
            class_module_mask = (
                self.df[Column.CLASS_MODULE] == self.context.full_module_name
            )
